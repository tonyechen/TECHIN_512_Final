...
if __name__ == "__main__":
    ...
    print("Rotate the encoder to test")
    print("Press Ctrl+C to exit")
    print()
    
    # Initialize encoder (adjust pins as needed)
    encoder = RotaryEncoder(board.D4, board.D5, pulses_per_detent=3)
    ...
    try:
        while True:
            if encoder.update():
                ...
                if delta > 0:
                    print(f"Position: {pos:4d} | Rotated RIGHT (+{delta})")
                elif delta < 0:
                    print(f"Position: {pos:4d} | Rotated LEFT  ({delta})")
        ...
        print()
        print("=== Test Complete ===")
        print(f"Final position: {encoder.position}")
        print("Exiting...")
